			  ...
			  <span>
	             <div class="btn-group">
				     ...
				     <ul class="dropdown-menu">
					       <li>
						      <a href="http://service.weibo.com/share/share.php?url=<%=rootPath%>/vistiorShowSubjectDatailFromShare?target=${subjectEntity.id }&pic=<%=imgurl %>${subjectEntity.faceimage }&title=${subjectEntity.name }&nbsp;&nbsp;${partEntity.info }" target="_blank">
						                     分享到新浪微博
						       </a>
					       </li>
						   <li>
							   <a href="http://share.v.t.qq.com/index.php?c=share&a=index&url=<%=rootPath%>/vistiorShowSubjectDatailFromShare?target=${subjectEntity.id }&pic=<%=imgurl %>${subjectEntity.faceimage }&title=${subjectEntity.name }&nbsp;&nbsp;${partEntity.info }" target="_blank">
							         分享到QQ微信
							   </a>
						   </li>
						   <li>
							   <a href="http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=<%=rootPath%>/index&pic=<%=imgurl %>${subjectEntity.faceimage }&title=${subjectEntity.name }&nbsp;&summary=${partEntity.info }|&nbsp;<%=rootPath%>/vistiorShowSubjectDatailFromShare?target=${subjectEntity.id }" target="_blank">
							         分享到QQ空间
							   </a>
						  </li>
		               </ul>
       ...
       <div class="span8 mxkplan mxkshadow">
         ...
         <div class="row" style="padding:5px;margin-bottom:5px;">
          ...
          <div class="span6">
            ...
            <button class="pull-right btn btn-primary btn-small" onclick="addTextComents('${subjectEntity.id}','${subjectEntity.userid }','subject')">评论</button>
          </div>
         </div>
         <hr />
	  ...
	  if(!confirm("确定要删除这个评论吗？")){
		  ...
		  $.ajax({
		   		...
		   		success : function(item) {
		   		    if(item == 'success'){
		 			   alert("删除成功！");
		 			   window.location.href= path + "/visitiorShowSubjectComements?target="+ partid;
				    }else if( item == 'error'){
				   	   alert("网络异常请重试");
				    }else {
				     	alert(item);
...
function addTextComents(commentedId,commentedUserId,traget){
	  ...
	  $.ajax({
	   		...
	   		success : function(item) {
	   		    if(item == 'success'){
	 			   alert("评论成功！");
	 			  window.location.href= path + "/visitiorShowSubjectComements?target=" + commentedId;
			    }else if( item == 'error'){
			   	   alert("网络异常请重试");
			    }else {
			     	alert(item);
