...
public class AnimatedGifEncoder {
	...
	private static final void analyzePixels(
		...
		NeuQuant nq = new NeuQuant(animatedGifEncoder.pixels, len,
				animatedGifEncoder.sample);
		...
		for (int i = 0; i < nPix; i++) {
			index = nq.map(animatedGifEncoder.pixels[k++] & 0xff,
		...
		if (animatedGifEncoder.transparent != null) {
			animatedGifEncoder.transIndex = findClosest(
					animatedGifEncoder.colorTab, animatedGifEncoder.usedEntry,
					animatedGifEncoder.transparent);
		}
	}

	private final static int findClosest(byte[] colorTab1,
			boolean[] usedEntry1, Color c) {
		if (colorTab1 == null)
			return -1;
		int r = c.getRed();
		int g = c.getGreen();
