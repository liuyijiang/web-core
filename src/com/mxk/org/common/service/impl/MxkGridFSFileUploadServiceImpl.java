...
public class MxkGridFSFileUploadServiceImpl implements MxkGridFSFileUploadService {
	...
	public String uploadImage(File file, String id, String type, String size,double max) {
		...
		try{
			 ...
			 gfsInput = new GridFS(db, getGridFsCollectionName(type)).createFile(file);//设定gridfs数据库位置
			 gfsInput.setFilename(id + size + type);//设定文件名
			 gfsInput.setContentType("png");//设定文件类型
			 gfsInput.save(); //save
	...
	public String uploadFile(File file, String id, String type) {
		...
		try{
			 gfsInput = new GridFS(db, getGridFsCollectionName(type)).createFile(file);//设定gridfs数据库位置
			 gfsInput.setFilename(id  + type);//设定文件名
			 gfsInput.save(); //save
